...
echo "flyway.url=jdbc:postgresql://$DATABASE_SERVER:5432/jibar_db" >> microservicedb.conf
...
flyway -configFiles=microservicedb.conf -table=jibar_schema_versions -sqlMigrationPrefix=V migrate
flyway -configFiles=microservicedb.conf -table=jibar_publications_versions -sqlMigrationPrefix=P migrate
                                         
flyway -configFiles=canonicaldb.conf    -table=jibar_schema_versions -sqlMigrationPrefix=V -locations=filesystem:/sql/migrations/ms migrate
flyway -configFiles=canonicaldb.conf    -table=jibar_canonial_versions -sqlMigrationPrefix=C -locations=filesystem:/sql/migrations/canonical migrate

sleep 5
                                         
flyway -configFiles=microservicedb.conf -table=jibar_data_versions -sqlMigrationPrefix=D migrate
