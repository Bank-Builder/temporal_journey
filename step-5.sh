...
PGPASSWORD=postgres psql -U postgres -p 7432 -h localhost -d canonical_db -c 'SELECT * FROM fica_status;'
PGPASSWORD=postgres psql -U postgres -p 7432 -h localhost -d canonical_db -c 'SELECT * FROM fica_status_history;'
echo "Change some data in the fica_db.fica_status table"
PGPASSWORD=postgres psql -U postgres -p 9432 -h localhost -d fica_db -c "UPDATE fica_status SET status = 'compliant', changed_by = 'leona' WHERE name = 'mr big';"
...
echo "jibar_db:jibar"
PGPASSWORD=postgres psql -U postgres -p 8432 -h localhost -d jibar_db -c '\d jibar;'
echo "canonical:jibar"
PGPASSWORD=postgres psql -U postgres -p 7432 -h localhost -d canonical_db -c '\d jibar;'
